  ...
  return(
    <div style={{...bgStyle,minHeight:"100vh",position:"relative",overflow:"hidden"}} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:520,margin:"0 auto",padding:"36px 20px 64px"}}>
            ...
            <div style={{background:C.surface,borderRadius:20,border:`1px solid ${C.border2}`,padding:"28px 24px 24px",position:"relative",overflow:"hidden"}}>
              ...
              <div style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:8,marginTop:22}}>
                {[
                  ...
                  {icon:"🌧",label:"Rain",  value:`${bh.p}%`,            score:bh.bd.precipitation},
                  ...
                  const fc=c||sc(score);
                  return(
                    <div key={label} style={{background:C.surface2,borderRadius:10,border:`1px solid ${C.border2}`,padding:"10px 8px",textAlign:"center",display:"flex",flexDirection:"column",gap:4}}>
              ...
              {/* 7-factor score bars — no weight % shown, actual values added */}
                ...
                <div key={label} style={{marginBottom:12,background:isBestWindow?`${fc}08`:C.surface2,borderRadius:10,border:`1px solid ${isBestWindow?fc+"30":C.border}`,padding:"12px 14px"}}>
                  ...
                  <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                    {group.map(h=>{
                      ...
                      return(
                        <div key={h.hr} style={{background:isThisBest?`${hfc}20`:C.surface,borderRadius:6,padding:"5px 8px",border:`1px solid ${isThisBest?hfc+"50":C.border}`,minWidth:54}}>
                          <div style={{...mono,fontSize:8,color:isThisBest?hfc:C.muted,letterSpacing:0.5}}>{fmt12(h.hr)}</div>
        ...
        <div style={{textAlign:"center",marginTop:10,...mono,fontSize:7,color:C.dim,letterSpacing:2,opacity:0.4}}>{weather.isLive?"LIVE · TOMORROW.IO + OPEN-METEO":"SEASONAL ESTIMATE"} · TEMPRUNTURE.COM</div>
      </div>
    </div>
  );
}
